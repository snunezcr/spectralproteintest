# Modified from molparser at https://github.com/eharpste/molparser
...
def apply_m_rad(line, mol):
    ...
    for i in range(0, len(line), 2):
        aaa = int(line[i])
        vvv = int(line[i + 1])
        mol[aaa]["radical"] = radical_dict[vvv]
    return mol
...
def apply_m_iso(line, mol):
    ...
    for i in range(0, len(line), 2):
        aaa = int(line[i])
        vvv = int(line[i] + 1)
        mol[aaa]["mass_diff"] = "+" + radical_dict[vvv]
    return mol
...
def parse_sdf_file(filename, verbose=False, data_items=False, tuple_realtions=False, n=-1):
   ...
   # mols = parse_sdf_file("./test.mol", True, True, True)
    #pprint(mols[0])